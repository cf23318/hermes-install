...
if ([string]::IsNullOrWhiteSpace($InstallerDir)) {
    $InstallerDir = (Get-Location).Path
}
...
$GatewayTaskName = "Hermes_Gateway"
$GatewayServiceDir = Join-Path $HermesHome "gateway-service"
$GatewayHiddenWrapper = Join-Path $GatewayServiceDir "Hermes_Gateway_hidden.pyw"
$BundleDir = ""
$OfflineBundleDir = Join-Path $HermesHome "offline-bundle"
$BundleStrict = $false
...
function Get-BundleDir {
    $parent = Split-Path -Parent $InstallerDir
    $grandParent = if (-not [string]::IsNullOrWhiteSpace($parent)) { Split-Path -Parent $parent } else { "" }
    $candidates = @(
        (Join-Path $InstallerDir "bundle\windows")
    )
    if (-not [string]::IsNullOrWhiteSpace($parent)) {
        $candidates += (Join-Path $parent "bundle\windows")
    }
    if (-not [string]::IsNullOrWhiteSpace($grandParent)) {
        $candidates += (Join-Path $grandParent "bundle\windows")
    }
...
function Add-UserPathEntries {
    ...
    foreach ($entry in $Entries) {
        ...
        foreach ($item in $items) {
            if ($item.TrimEnd([char[]]"\/") -ieq $entry.TrimEnd([char[]]"\/")) {
                $exists = $true
                break
            }
        }
    ...
    if ($changed) {
        ...
        Write-Info "已写入用户 PATH，新的 PowerShell 窗口会自动识别胖包运行环境。"
    }
}

function Add-SessionPathEntries {
    param([string[]]$Entries)

    $items = if ($env:Path) { @($env:Path -split ";") } else { @() }
...
function Install-BundledPlaywrightChromium {
    if ([string]::IsNullOrWhiteSpace($script:BundleDir)) { return }

    $source = Join-Path $script:BundleDir "ms-playwright"
    if (-not (Test-Path $source)) { return }

    Write-Info "检测到胖包 Playwright Chromium 浏览器资源。"
    if (Test-PlaywrightChromiumInstalled) {
        Write-Info "本机 Playwright Chromium 已存在，跳过复制。"
        return
    }
    ...
    Write-Info "已复制胖包 Playwright Chromium 到: $target"
}

function Install-BundledNode {
    if ([string]::IsNullOrWhiteSpace($script:BundleDir)) { return }

    $source = Join-Path $script:BundleDir "node"
    if (-not (Test-Path (Join-Path $source "node.exe"))) { return }

    $target = Join-Path $HermesHome "node"
    Remove-Item -Recurse -Force $target -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Force -Path $HermesHome | Out-Null
    Copy-Item -Path $source -Destination $target -Recurse -Force
    Add-SessionPathEntries @($target)
    Add-UserPathEntries @($target)
    Write-Info "已安装胖包 Node 到: $target"
}

function Install-BundledGit {
    if ([string]::IsNullOrWhiteSpace($script:BundleDir)) { return }

    $source = Join-Path $script:BundleDir "git"
    if (-not (Test-Path (Join-Path $source "cmd\git.exe"))) { return }

    $target = Join-Path $HermesHome "git"
    Remove-Item -Recurse -Force $target -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Force -Path $HermesHome | Out-Null
    Copy-Item -Path $source -Destination $target -Recurse -Force
    Add-SessionPathEntries @("$target\cmd", "$target\bin", "$target\usr\bin")
    Add-UserPathEntries @("$target\cmd", "$target\bin", "$target\usr\bin")

    $bash = Join-Path $target "bin\bash.exe"
    if (-not (Test-Path $bash)) {
        $bash = Join-Path $target "usr\bin\bash.exe"
    }
    if (Test-Path $bash) {
    ...
    Write-Info "已安装胖包 Git 到: $target"
}

function Install-BundledUv {
    if ([string]::IsNullOrWhiteSpace($script:BundleDir)) { return }

    $source = Join-Path $script:BundleDir "uv\uv.exe"
    if (-not (Test-Path $source)) {
        $source = Join-Path $script:BundleDir "uv\bin\uv.exe"
    }
    if (-not (Test-Path $source)) { return }

    $targetDir = Join-Path $env:USERPROFILE ".local\bin"
    $target = Join-Path $targetDir "uv.exe"
    New-Item -ItemType Directory -Force -Path $targetDir | Out-Null
    Copy-Item -Path $source -Destination $target -Force
    Add-SessionPathEntries @($targetDir)
    Add-UserPathEntries @($targetDir)
    Write-Info "已安装胖包 uv 到: $target"
}

function Copy-BundleDirIfExists {
    param(
        [string]$SourceRoot,
        [string]$SubPath,
        [string]$TargetRoot
    ...
    $source = Join-Path $SourceRoot $SubPath
    if (-not (Test-Path $source)) { return $false }

    $target = Join-Path $TargetRoot $SubPath
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $target) | Out-Null
    Copy-Item -Path $source -Destination $target -Recurse -Force
    return $true
}

function Copy-BundleFileIfExists {
    param(
        [string]$SourceRoot,
        [string]$SubPath,
        [string]$TargetRoot
...
function Stage-BundledInstallAssets {
    param([string]$SourceBundle)

    $copied = $false
    New-Item -ItemType Directory -Force -Path $HermesHome | Out-Null
    Remove-Item -Recurse -Force $script:OfflineBundleDir -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Force -Path $script:OfflineBundleDir | Out-Null

    if (Copy-BundleFileIfExists $SourceBundle "manifest.json" $script:OfflineBundleDir) { $copied = $true }
    if (Copy-BundleDirIfExists $SourceBundle "uv-cache" $script:OfflineBundleDir) { $copied = $true }
    if (Copy-BundleDirIfExists $SourceBundle "npm-cache" $script:OfflineBundleDir) { $copied = $true }
    if (Copy-BundleDirIfExists $SourceBundle "wheelhouse" $script:OfflineBundleDir) { $copied = $true }

    if ($copied) {
        Write-Info "已导入胖包离线安装资源到: $script:OfflineBundleDir"
    } else {
        Remove-Item -Recurse -Force $script:OfflineBundleDir -ErrorAction SilentlyContinue
    }
}

function Use-BundledResources {
    $sourceBundle = Get-BundleDir
    if ([string]::IsNullOrWhiteSpace($sourceBundle)) { return }

    $script:BundleDir = $sourceBundle
    Write-Info "检测到胖包资源: $sourceBundle"
    $manifest = Join-Path $sourceBundle "manifest.json"
    if (Test-Path $manifest) {
        $script:BundleStrict = $true
        Write-Info "检测到完整胖包 manifest，将优先使用内置资源完成安装。"
    }

    Install-BundledNode
    Install-BundledGit
    Install-BundledUv
    Install-BundledPlaywrightChromium
    Stage-BundledInstallAssets $sourceBundle

    if (Test-Path $script:OfflineBundleDir) {
        $script:BundleDir = $script:OfflineBundleDir
    }

    $npmCache = Join-Path $script:BundleDir "npm-cache"
    if (Test-Path $npmCache) {
        $env:npm_config_cache = $npmCache
        Write-Info "已使用胖包 npm 缓存: $npmCache"
    }
    $uvCache = Join-Path $script:BundleDir "uv-cache"
    if (Test-Path $uvCache) {
        $env:UV_CACHE_DIR = $uvCache
        Write-Info "已使用胖包 uv 缓存: $uvCache"
    }
    if ($script:BundleStrict) {
        $env:npm_config_prefer_offline = "true"
        Write-Info "胖包环境资源已安装，将优先使用本地缓存。"
    }
}
...
function Backup-GatewayServiceArtifacts {
    New-Item -ItemType Directory -Force -Path $GatewayServiceDir | Out-Null
    $backupDir = Join-Path $GatewayServiceDir "backups"
    ...
    if (Get-Command Get-ScheduledTask -ErrorAction SilentlyContinue) {
        try {
            ...
            if ($task) {
                $taskBackup = Join-Path $backupDir ("{0}-{1}.xml" -f $GatewayTaskName, $stamp)
                Export-ScheduledTask -TaskName $GatewayTaskName | Out-File -FilePath $taskBackup -Encoding UTF8
                Write-Info "已备份旧 Gateway Scheduled Task: $taskBackup"
            }
        } catch {
            Write-Warn2 "备份旧 Gateway Scheduled Task 失败: $_"
        }
    }

    $oldFiles = @(
        (Join-Path $GatewayServiceDir "Hermes_Gateway.cmd"),
        (Join-Path $GatewayServiceDir "Hermes_Gateway.ps1"),
        (Join-Path $GatewayServiceDir "Hermes_Gateway.pyw"),
        $GatewayHiddenWrapper
    )
    foreach ($file in $oldFiles) {
        if (Test-Path $file) {
            $backup = Join-Path $backupDir ("{0}.{1}.bak" -f ([IO.Path]::GetFileName($file)), $stamp)
            Copy-Item $file $backup -Force
            Write-Info "已备份旧 Gateway 启动脚本: $backup"
        }
    }
}

function Get-PythonwCommand {
    $pythonw = Join-Path $InstallDir "venv\Scripts\pythonw.exe"
    if (Test-Path $pythonw) { return $pythonw }

    $python = Join-Path $InstallDir "venv\Scripts\python.exe"
    if (Test-Path $python) {
        Fail "未找到 pythonw.exe，不能隐藏启动 Gateway。已找到 python.exe: $python"
    }

    Fail "未找到 Hermes Python venv，不能隐藏启动 Gateway。"
}

function Write-HiddenGatewayWrapper {
    param(
        [string]$Hermes
    )
...
hermes = install_dir / "venv" / "Scripts" / "hermes.exe"
log_dir = hermes_home / "logs"
service_dir = hermes_home / "gateway-service"
log_dir.mkdir(parents=True, exist_ok=True)
service_dir.mkdir(parents=True, exist_ok=True)

diag_path = service_dir / "gateway-hidden-launch.log"
...
if not hermes.exists():
    diag(f"missing hermes executable: {hermes}")
    sys.exit(2)

stdout_path = log_dir / "gateway.hidden.log"
stderr_path = log_dir / "gateway.hidden.error.log"
env = os.environ.copy()
...
try:
    with stdout_path.open("ab", buffering=0) as stdout, stderr_path.open("ab", buffering=0) as stderr:
        ...
        (service_dir / "gateway-hidden.pid").write_text(str(proc.pid), encoding="utf-8")
        diag(f"started hidden gateway pid={proc.pid}")
        return_code = proc.wait()
    diag(f"gateway exited pid={proc.pid} code={return_code}")
    sys.exit(return_code)
except Exception as exc:
    diag(f"failed to start hidden gateway: {exc!r}")
    raise
'@

    [System.IO.File]::WriteAllText($GatewayHiddenWrapper, $wrapper, (Get-Utf8NoBomEncoding))
    Write-Info "已写入隐藏 Gateway 启动器: $GatewayHiddenWrapper"
}

function Install-HiddenGatewayTask {
    param(
        [string]$Hermes
    )

    Backup-GatewayServiceArtifacts
    $pythonw = Get-PythonwCommand
    Write-HiddenGatewayWrapper $Hermes

    if (-not (Get-Command Register-ScheduledTask -ErrorAction SilentlyContinue)) {
        Fail "当前系统缺少 Register-ScheduledTask，不能创建隐藏 Gateway 计划任务。"
    }

    $userId = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name
    $action = New-ScheduledTaskAction -Execute $pythonw -Argument "`"$GatewayHiddenWrapper`""
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description "Start Hermes Gateway hidden via pythonw.exe." `
        -Force | Out-Null

    Write-Info "已安装隐藏 Gateway Scheduled Task: $GatewayTaskName"
    return $pythonw
}

function Start-HiddenGatewayTask {
    param(
        [string]$Hermes,
        [string]$Pythonw
    )

    Write-Info "通过隐藏 Scheduled Task 启动 Hermes Gateway。"
    ...
    Write-Warn2 "Scheduled Task 启动后暂未检测到 Gateway，直接用 pythonw.exe 再尝试一次。"
    Start-Process -FilePath $Pythonw -ArgumentList @($GatewayHiddenWrapper) -WindowStyle Hidden
    Start-Sleep -Seconds 5
}
...
function Install-FeishuDependencies {
    ...
    $wheelhouse = if (-not [string]::IsNullOrWhiteSpace($script:BundleDir)) { Join-Path $script:BundleDir "wheelhouse" } else { "" }
    if (-not [string]::IsNullOrWhiteSpace($wheelhouse) -and (Test-Path $wheelhouse)) {
        Write-Info "优先使用胖包 Python wheelhouse: $wheelhouse"
        $offlineArgs = @("pip", "install", "--python", $python, "--no-index", "--find-links", $wheelhouse) + $missingPackages
        & $uvExe @offlineArgs
        if ($LASTEXITCODE -eq 0) {
            Write-Info "国内聊天工具依赖安装完成。"
            return
        }
        Write-Warn2 "胖包 wheelhouse 安装失败，改用在线安装。"
    }
...
function Start-Gateway {
    ...
    Write-Info "安装或刷新隐藏 Gateway 后台任务（pythonw.exe + Scheduled Task）。"
    $pythonw = Install-HiddenGatewayTask $hermes
    Start-HiddenGatewayTask $hermes $pythonw

    & $hermes gateway status
    & $hermes gateway list

    Write-Info "Gateway 已通过隐藏后台任务启动；可以关闭本安装器 PowerShell 窗口。"
    Write-Info "隐藏启动日志: $(Join-Path $GatewayServiceDir 'gateway-hidden-launch.log')"
    Write-Info "Gateway 输出日志: $(Join-Path $HermesHome 'logs\gateway.hidden.log')"
}
...
function Main-Menu {
    while ($true) {
        ...
        switch ($choice) {
            ...
            "6" { Configure-Proxy; Use-BundledResources; Install-FeishuDependencies; Configure-Messaging; Start-Gateway; Pause-Installer }
            "7" { Configure-Proxy; Use-BundledResources; Install-BrowserTools; Pause-Installer }
