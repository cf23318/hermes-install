...
```text
dist/HermesAgentInstaller.command
dist/HermesAgentInstaller.ps1
```

备用 zip 也会生成，但当前网页主推直接源文件：

```text
dist/HermesAgentInstaller-mac.zip
dist/HermesAgentInstaller-windows.zip
```
...
- 复制 mac 安装器到 `dist/HermesAgentInstaller.command`
- 复制 Windows 安装器到 `dist/HermesAgentInstaller.ps1`
- 保留两个 zip 备用
...
- `package.sh` 强制 `HermesAgentInstaller.ps1` 和 `dist/HermesAgentInstaller.ps1` 保存为 UTF-8 BOM
...
```text
mac/HermesAgentInstaller.command
windows/HermesAgentInstaller.ps1
web/index.html
password.html
package.sh
README.md
...
```bash
bash -n mac/HermesAgentInstaller.command
...
print(Path("dist/HermesAgentInstaller.ps1").read_bytes().startswith(b"\xef\xbb\xbf"))
PY
```
...
```bash
unzip -l dist/HermesAgentInstaller-mac.zip
unzip -l dist/HermesAgentInstaller-windows.zip
```
